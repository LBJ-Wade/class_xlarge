...
int input_read_parameters(
...
#ifdef _DAM_MPI
    if((flag1 == _FALSE_) || (int1<=0)) {
      mpi_abort(1,"Selection means were not provided\n");
    }
    else {
      mpi_share_bins(int1);
#ifdef _DAM_DEBUG
      printf("Node %d: %d bins, starting from %d\n",Mpi_this_node,
	     Mpi_nbins_here,Mpi_bin0);
#endif //_DAM_DEBUG
      ...
      for(i=0;i<Mpi_nbins_here;i++) {
        class_test((pointer1[Mpi_bin0+i] < 0.) || (pointer1[Mpi_bin0+i] > 1000.),
                   errmsg,
                   "input of selection functions: you asked for a mean redshift equal to %e, sounds odd",
                   pointer1[Mpi_bin0+i]);
        ptr->selection_mean[i] = pointer1[Mpi_bin0+i];
      }
      ...
      if((flag1==_FALSE_) || (int1!=Mpi_nbins_total)) {
	mpi_abort(1,"Selection widths were incorrectly provided\n");
      }
      else {
	for(i=0;i<Mpi_nbins_here;i++) {
	  class_test((pointer1[Mpi_bin0+i] < 0.) || (pointer1[Mpi_bin0+i] > 1000.),
		     ...
		     pointer1[Mpi_bin0+i]);
	  ptr->selection_width[i] = pointer1[Mpi_bin0+i];
#ifdef _DAM_DEBUG
	  printf("Node %d: bin %d mean %lf, width %lf\n",Mpi_this_node,Mpi_bin0+i,
  ...
  sprintf(ppt->root,"%s_proc%d",pop->root,Mpi_this_node);
